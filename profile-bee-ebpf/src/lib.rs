...
use profile_bee_common::{
    FramePointers, StackInfo, EVENT_TRACE_ALWAYS,
...
pub unsafe fn collect_trace<C: EbpfContext>(ctx: C) {
    ...
    let (ip, bp, len) = if dwarf_enabled() {
        dwarf_copy_stack(&ctx, &mut pointer.pointers, tgid)
    } else {
...
unsafe fn dwarf_copy_stack<C: EbpfContext>(ctx: &C, pointers: &mut [u64], tgid: u32) -> (u64, u64, usize) {
    ...
    let proc_info = match PROC_INFO.get(&proc_key) {
        ...
        None => {
            ...
            return copy_stack(ctx, pointers);
        }
    };
    ...
    while i < max_depth {
        ...
        sp = cfa;  // CFA of current frame = SP of previous frame
        bp = new_bp;
...
/// Returns the entry whose PC range covers the given relative_pc
#[inline(always)]
unsafe fn binary_search_unwind_entry(table_start: u32, table_count: u32, relative_pc: u64) -> Option<UnwindEntry> {
    ...
    // Binary search - bounded iterations for eBPF verifier (log2(250000) ≈ 18)
    ...
    // lo-1 is the last entry with pc <= relative_pc
    if lo == 0 {
        return None;
    }
