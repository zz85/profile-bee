...
# Profile a Bun script. Uses `-- bun <script>` so setup_process_to_profile
# auto-injects BUN_JSC_useJITDump=1 (JSC writes jit-<pid>.dump).
run_bun_profiler() {
    local script="$1"
    local name="$2"
    shift 2
...
test_bun_jitdump_callstack() {
    ...
    # JSC emits JITDump symbols like "JSC-FTL: hot" / "JSC-DFG: processData"
    # (JSC #hash suffixes are stripped during formatting). Require a JSC JIT
    # tier marker before a fixture function name so we don't match native syms.
    assert_stack_contains "$file" \
