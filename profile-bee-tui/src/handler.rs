...
use crate::{
    app::{App, AppResult, FlameGraphInput, InputBuffer},
    state::ViewKind,
};
...
pub fn handle_command_generic(key_event: KeyEvent, app: &mut App) -> AppResult<bool> {
    ...
    match key_event.code {
        ...
        KeyCode::Char('[') if matches!(app.flamegraph_input, FlameGraphInput::Live) => {
            if app.flamegraph_state().view_kind == ViewKind::Heatmap {
