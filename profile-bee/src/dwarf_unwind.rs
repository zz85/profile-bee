...
mod tests {
    ...
    fn test_array_of_maps_pattern() {
        ...
        assert!(num_shards > 0, "Expected at least one shard after loading process");
        assert!(num_shards <= MAX_UNWIND_SHARDS, 
                "Should not exceed MAX_UNWIND_SHARDS ({})", MAX_UNWIND_SHARDS);
        
        // Verify each shard contains unwind entries
        for (shard_id, entries) in manager.binary_tables.iter().enumerate() {
            assert!(!entries.is_empty(), 
                    "Shard {} should contain entries after loading", shard_id);
            
            // Verify entries are sorted by PC (required for binary search in eBPF)
            for window in entries.windows(2) {
                assert!(window[0].pc <= window[1].pc,
                        "Entries in shard {} not sorted: {} > {}", 
                        shard_id, window[0].pc, window[1].pc);
            }
        }
        ...
        assert_eq!(new_shards.len(), 0, "Expected no new shards on refresh");
        assert_eq!(manager.binary_tables.len(), old_len, 
