...
- **async-profiler engine (experimental)** — `--java-engine async-profiler` delegates Java stacks to [async-profiler](https://github.com/async-profiler/async-profiler) (AsyncGetCallTrace), so JIT/interpreter/inlined frames resolve **without `-XX:+PreserveFramePointer`**. Works per-`--pid` or system-wide (attaches every discovered JVM, eBPF for the rest). `asprof` is used from `$ASPROF`, `$PATH`, `/opt/async-profiler`, `/usr/local/async-profiler`, or the cache if present; only when none is found is the pinned release auto-downloaded (SHA-256-verified) into a world-readable cache. Batch output only; see below
